...
function handleCalBtn(btnValue,numValue) { // 계산(덧셈)버튼을 눌렀을 때 실행되는 메서드
    if (iseq) { // = 계산이 끝난 후 다시 연산자를 누르면 num2초기화
        num2 = numValue // 곱셈연산의 경우에는 num2를 0으로 초기화 시켜버리면 숫자를 제외한 연산기호나 =을 클릭했을때 결과가 0이 됨
        iseq = false
    }
